<template>
  <div class="storeTitle">
    <div class="titleBox">
      <strong><span>스토어.</span>좋아하는 Apple 제품을 <br />구입하는 가장 좋은 방법.</strong>
      <div class="askBox">
        <div class="ask">
          <div>
            <img
...
<style scoped>
.storeTitle {
  @apply relative w-full bg-[#f5f5f7];
}
.titleBox {
  @apply p-[80px_12px_64px] flex justify-start items-center;
  margin-inline: calc(-268.46154px + 28.36538vw);
}
.titleBox strong {
  @apply text-[#6E6E73] text-[46px] font-bold leading-[1.1875];
}
.titleBox strong span {
  @apply text-[#1d1d1f];
}
.askBox{
  @apply absolute right-[40px] top-[80px] flex flex-col gap-[16px];
}
.ask{
  @apply flex justify-start items-start gap-[8px];
}
.txt span{
  @apply block text-[14px] font-semibold leading-[22px] text-[#15151f];
}
.txt button{
  @apply block text-[14px] font-medium leading-[22px] text-[#0066cc];
}
.txt button:hover{
  @apply underline;
}
